...
export function TopHeader({ title, subtitle, children }: { title?: string, subtitle?: string, children?: React.ReactNode }) {
    return (
        <header className="min-h-20 py-3 md:py-0 flex flex-wrap items-center justify-between gap-y-3 gap-x-4 px-4 md:px-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30 flex-shrink-0">
            <div className="flex items-center gap-4">
                ...
                {title && (
                    <div>
                        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{title}</h1>
